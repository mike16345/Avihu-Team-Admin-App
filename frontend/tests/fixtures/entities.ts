...
export async function mockAuthSuccess(page: Page) {
  ...
  await page.route(apiMatchers.userSession, async (route) =>
    fulfillJson(route, { data: { isValid: true }, message: "ok" })
  );
}
...
export async function mockAuthFailure(page: Page) {
  if (isLiveMode) return;
  await page.route(apiMatchers.userLogin, async (route) =>
    fulfillJson(route, { message: "Invalid credentials" }, 401)
  );
}
...
export async function mockCreateUserSuccess(
  page: Page,
  store: UserFixture[],
  createdUser: UserFixture
) {
  if (isLiveMode) return;
  await page.route(apiMatchers.users, async (route) => {
