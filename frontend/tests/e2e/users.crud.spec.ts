...
async function mockUserDashboard(page: Page, userId: string, detail = createUserResponse) {
  ...
  await interceptJSON(
    page,
    "GET",
    new RegExp(`/weighIns/weights/user\\?id=${userId}`),
    emptyWeighIns
  );
  await interceptJSON(page, "GET", new RegExp(`/recordedSets/user\\?userId=${userId}`), {
    data: [],
    message: "ok",
  });
  ...
  await interceptJSON(page, "GET", new RegExp(`/userImageUrls/user\\?userId=${userId}`), {
    data: [],
    message: "ok",
  });
}
...
test.describe("User CRUD", () => {
  ...
  test("delete created user (only if created)", async ({ page }) => {
    ...
    await interceptJSON(page, "GET", new RegExp(`/userImageUrls/user\\?userId=${createdUserId}`), {
      data: [],
      message: "ok",
    });
    await interceptJSON(
      page,
      "DELETE",
      new RegExp(`/users/one\\?id=${createdUserId}`),
      deleteUserResponse
    );
