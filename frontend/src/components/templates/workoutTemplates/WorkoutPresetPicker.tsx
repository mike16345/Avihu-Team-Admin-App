...
import { ActiveChip, FilterSection } from "./WorkoutPresetPickerPrimitives";
...
const getPickerTitle = (preview: Preset | null) => {
  if (!preview) return "בחירת תבנית אימון";
  return preview.name || "תצוגת תבנית";
};

const getPickerSubtitle = (preview: Preset | null) => {
...
const getFavoritesButtonClassName = (favoritesOnly: boolean) => {
  if (favoritesOnly) {
...
const getFavoritesCountClassName = (favoritesOnly: boolean) => {
  if (favoritesOnly) return "bg-amber-500 text-white";
  return "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300";
};

const getFilterOptionButtonClassName = (active: boolean) => {
...
const getWeekFilterOptionButtonClassName = (active: boolean) =>
...
const getWorkoutFrequency = (preset: Preset) => {
  if (typeof preset.workoutsPerWeek === "number") return preset.workoutsPerWeek;
  return -1;
};

const getFavoriteSortWeight = (preset: Preset, isFavorite: (presetId?: string) => boolean) => {
...
const WorkoutPresetPicker: React.FC<WorkoutPresetPickerProps> = ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        dir="rtl"
        className="!max-w-[1280px] w-[95vw] h-[92vh] max-h-[92vh] overflow-hidden p-0 gap-0 rounded-2xl flex flex-col font-heebo"
      >
            ...
            <div className="flex flex-1 flex-col overflow-hidden bg-slate-50/60 dark:bg-slate-950/40">
              <div className="flex flex-wrap items-center gap-3 border-b border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 px-6 py-3">
                ...
                <button
                  ...
                  className={`inline-flex h-9 items-center gap-2 rounded-xl border px-3 text-xs font-bold transition-all ${getFavoritesButtonClassName(
                    favoritesOnly
                  )}`}
                >
                  <FaStar size={11} />
                  ...
                  {favoritesCount > 0 && (
                    <span
                      className={`inline-flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-[10px] font-bold ${getFavoritesCountClassName(
                        favoritesOnly
                      )}`}
                    >
                      {favoritesCount}
                ...
                {!hasNoResults && (
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {filtered.map((p) => {
                      ...
                      const taggedMuscles = p.muscleFocus ?? [];
                      const hasTaggedMuscles = taggedMuscles.length > 0;
                      const hasRawMuscles = !hasTaggedMuscles && muscles.length > 0;
                      return (
                        <button
                          key={p._id || p.name}
                          type="button"
