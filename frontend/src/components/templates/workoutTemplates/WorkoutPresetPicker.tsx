...
const WorkoutPresetPicker: React.FC<WorkoutPresetPickerProps> = ({
  ...
  const levelLabelOf = (v: WorkoutLevel) =>
    LEVEL_OPTIONS.find((o) => o.value === v)?.label ?? v;
  const goalLabelOf = (v: WorkoutGoal) =>
  ...
  const durationLabelOf = (v: string) =>
    DURATION_OPTIONS.find((o) => o.value === v)?.label ?? v;
  ...
  const durationMatches = (
    dur: number,
    bands: Exclude<DurationFilter, "all">[]
  ): boolean => {
  ...
  }, [presets, search, weekFilter, durationFilter, levelFilter, goalFilter, equipmentFilter, muscleFilter, favoritesOnly, isFavorite]);
      ...
      >
            ...
            {favoritesCount > 0 && (
              <span
                className={`inline-flex h-5 min-w-[20px] items-center justify-center rounded-full px-1.5 text-[10px] font-bold ${
                  favoritesOnly ? "bg-amber-500 text-white" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"
                }`}
              >
                {favoritesCount}
              </span>
        ...
        {hasActiveFilters && (
          <div className="flex flex-wrap items-center gap-1.5 border-b border-slate-200 dark:border-slate-800 bg-white/70 dark:bg-slate-900/60 px-6 py-2.5">
            {weekFilter.map((w) => (
              <ActiveChip key={`w-${w}`} label={`${w}× בשבוע`} onRemove={() => setWeekFilter((p) => p.filter((x) => x !== w))} />
            ))}
            {durationFilter.map((d) => (
              <ActiveChip key={`d-${d}`} label={durationLabelOf(d)} onRemove={() => setDurationFilter((p) => p.filter((x) => x !== d))} />
            ))}
            {levelFilter.map((l) => (
              <ActiveChip key={`l-${l}`} label={levelLabelOf(l)} onRemove={() => setLevelFilter((p) => p.filter((x) => x !== l))} />
            ))}
            {goalFilter.map((g) => (
              <ActiveChip key={`g-${g}`} label={goalLabelOf(g)} onRemove={() => setGoalFilter((p) => p.filter((x) => x !== g))} />
            ))}
            {equipmentFilter.map((e) => (
              <ActiveChip key={`e-${e}`} label={equipmentLabelOf(e)} onRemove={() => setEquipmentFilter((p) => p.filter((x) => x !== e))} />
            ))}
            {muscleFilter.map((m) => (
              <ActiveChip key={`m-${m}`} label={muscleLabelOf(m)} onRemove={() => setMuscleFilter((p) => p.filter((x) => x !== m))} />
            ))}
            <button
              type="button"
              onClick={clearAll}
