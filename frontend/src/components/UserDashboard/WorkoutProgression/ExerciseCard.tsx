...
export const getExerciseCardId = (exercise: FlatExercise) => `${exercise.group}-${exercise.name}`;
...
const getExpandButtonState = (isExpanded: boolean) => {
  if (isExpanded) return { Icon: FaChevronUp, label: "הסתר היסטוריה" };
  return { Icon: FaChevronDown, label: "ראה היסטוריה מלאה" };
};
...
export function ExerciseCard({
  ...
  const monthGainPercent = monthValue === 0 ? 0 : Math.round((monthGain / monthValue) * 100);
  ...
  const sparklineValues = monthSessions.map((session) => getSessionValue(session, isBodyweight));
  ...
  return (
    ...
    >
      <button onClick={() => onOpen(exercise)} className="w-full p-4 text-right">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-1">
              <span className="inline-flex items-center rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-0.5 text-[10px] font-bold text-slate-700 dark:text-slate-200">
                {positionLabel}
              </span>
              <span
      ...
      <button
        onClick={(event) => {
          ...
          onExpandedChange(getNextExpandedCardId(isExpanded, cardId));
        }}
                ...
                .map(({ session, index }) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className={`py-1.5 text-center font-semibold ${historyValueClassName}`}>
...
function MetricColumn({ label, dateLabel, value, unit, secondary, tone }: MetricColumnProps) {
