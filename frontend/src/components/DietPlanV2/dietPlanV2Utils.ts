...
export const searchFoodLibrary = (
  ...
  if (!normalised) {
    const pool = kind ? MOCK_FOOD_LIBRARY.filter((food) => food.kind === kind) : MOCK_FOOD_LIBRARY;
    return pool.slice(0, limit);
  }

  const scored = MOCK_FOOD_LIBRARY.filter((food) => (kind ? food.kind === kind : true))
  ...
  const wide = MOCK_FOOD_LIBRARY.filter((food) => !kind || food.kind !== kind)
    .map((food) => ({ food, score: scoreFoodMatch(food, normalised) }))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score || a.food.name.length - b.food.name.length);
...
export const isPlateDescription = (trimmed: string): boolean => {
  const tokens = trimmed.replace(/[,،]/g, " ").replace(/\s+/g, " ").split(" ").filter(Boolean);
...
const bestFoodMatch = (
  ...
  const sameKind = MOCK_FOOD_LIBRARY.filter((food) => food.kind === categoryKind)
  ...
  const wide = MOCK_FOOD_LIBRARY.map((food) => ({ food, score: scoreFoodMatch(food, cleaned) }))
    .filter((entry) => entry.score >= QUICK_MATCH_CONFIDENCE_FLOOR)
    .sort((a, b) => b.score - a.score || a.food.name.length - b.food.name.length);
