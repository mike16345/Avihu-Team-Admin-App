...
const cloneMeal = (meal: DietV2Meal): DietV2Meal => ({
  ...
  addOns: meal.addOns && meal.addOns.length > 0 ? meal.addOns.map((item) => ({ ...item })) : undefined,
...
const DietPlanV2Editor: React.FC<DietV2EditorProps> = ({
  ...
  const hasPlanItems = plan.meals.some(
    (meal) =>
      meal.categories.some((category) => category.items.length > 0) || (meal.addOns?.length ?? 0) > 0
  );
