...
export const UserDashboard = () => {
  ...
  const [progressSub, setProgressSub] = useState<ProgressSubTab>("weight");
  ...
  const statusColors: Record<AccountStatus, { border: string; bg: string; text: string; dot: string }> = {
    active: { border: "border-emerald-200", bg: "bg-emerald-50", text: "text-emerald-700", dot: "bg-emerald-500" },
    user: { border: "border-blue-200", bg: "bg-blue-50", text: "text-blue-700", dot: "bg-blue-500" },
    disabled: { border: "border-red-200", bg: "bg-red-50", text: "text-red-700", dot: "bg-red-500" },
  };
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      {mainTab === "profile" && (
        <div className="w-full max-w-3xl rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
      ...
      {mainTab === "workout" && (
        <div className="rounded-2xl border border-slate-200/80 bg-white p-8 text-center shadow-sm">
          <FaDumbbell size={32} className="mx-auto mb-3 text-blue-600" />
          <h3 className="mb-2 text-lg font-bold text-slate-900">תוכנית אימונים</h3>
          <p className="mb-4 text-sm text-slate-500">
            לעריכת תוכנית האימונים של {currentUser?.firstName}
          </p>
          <button
            onClick={() => navigate(`/workout-plans/${currentUser?._id}`)}
      ...
      {mainTab === "diet" && (
        <div className="rounded-2xl border border-slate-200/80 bg-white p-8 text-center shadow-sm">
          <FaAppleWhole size={32} className="mx-auto mb-3 text-emerald-600" />
          <h3 className="mb-2 text-lg font-bold text-slate-900">תפריט תזונה</h3>
          <p className="mb-4 text-sm text-slate-500">
            לעריכת תפריט התזונה של {currentUser?.firstName}
          </p>
          <button
            onClick={() => navigate(`/diet-plans/${currentUser?._id}`)}
            className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-700"
          >
            פתח את תפריט התזונה ←
          </button>
    ...
    >
      ...
      >
        ...
        <p className="mb-3 text-sm text-slate-700">
          ...
          <span className="font-bold text-slate-900">{userName}</span> מ-
    ...
    >
      ...
      >
        ...
        <p className="mb-4 text-sm text-slate-600">
          אתה עומד לשנות את הסטטוס של <span className="font-bold text-slate-900">{userName}</span>{" "}
          מ-
...
function ProfileField({
  ...
  return (
    <div>
      ...
      <div
        dir={dir}
        className="rounded-xl border border-slate-200 bg-slate-50/60 px-4 py-2.5 text-sm text-slate-800"
      >
        {value || "—"}
      </div>
    </div>
